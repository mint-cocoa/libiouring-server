    ...
    bool& in_game)
{
    ...
    case game::C_ENTER_GAME: {
        ...
        *res.mutable_player() = MakePlayerInfo(ps);
        ...
        for (auto& [pid, existing] : zone->PlayersSnapshot()) {
    ...
    case game::C_MOVE: {
        ...
        // Update position under zone lock
        zone->WithPlayer(player_id, [&](PlayerState& ps) {
            if (req.has_position()) {
    ...
    case game::C_CREATE_ROOM: {
        ...
        if (old_zone) {
            old_zone->RemovePlayer(player_id, pool);
        }
        ...
        new_zone->AddPlayer(player_id, ps, pool);
        ...
        *res.mutable_player() = MakePlayerInfo(ps);
        ...
        auto buf = BuildPacket(pool, game::S_CREATE_ROOM, res);
        if (buf) session.Send(std::move(buf));
        break;
    }
    case game::C_JOIN_ROOM: {
        ...
        if (!target_zone) {
            ...
            auto buf = BuildPacket(pool, game::S_JOIN_ROOM, res);
            if (buf) session.Send(std::move(buf));
            return;
        }
        ...
        game::S_PlayerList player_list;
        for (auto& [pid, existing] : target_zone->PlayersSnapshot()) {
            *player_list.add_players() = MakePlayerInfo(existing);
        }
        auto list_buf = BuildPacket(pool, game::S_PLAYER_LIST, player_list);
        if (list_buf) session.Send(list_buf);

        target_zone->AddPlayer(player_id, ps, pool);
